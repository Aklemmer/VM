...
;;;
;;; A set of procedures that allow easy output (via a print() procedure) to the console.  Includes code for scrolling the console.
;;; These procedures should be copied-and-pasted into existing BIOS or kernel code.  They assume no fixed loading location, using
;;; all IP-relative label addresses.  They also assume that you can pass console base/limit addresses (obtained from the bus
;;; controller's device table, presumably), as well as pointers to strings to be emitted.
;;;
...
;;; ================================================================================================================================
;;; Entry point.  This dummy __start() is here just for testing.  Do not copy-and-paste it into your code; it's not part of the
;;; library of procedures for your code to use.
...
RAM_search_failure:
	...
	COPY		%G5		*+_static_error_RAM_not_found
	HALT
...
RAM_found:
	...
	MULUS		%G4		*+_static_KB_size *+_static_bytes_per_KB ; %G4 = |kmem|
	...
	COPY		*+_static_base			%G1
	COPY		*+_static_limit			%SP
	...
	COPY		%G5		*+_static_error_main_returned
	HALT

RAM_too_small:
	;; Set an error code and halt.
	COPY		%G5		*+_static_error_small_RAM
	HALT
;;; ================================================================================================================================
